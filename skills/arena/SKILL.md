...
3. Pick the runners. Default to one each on `fable`, `opus`, `sonnet` (Claude Code exposes only Claude-family models; candidate diversity comes from independent contexts and, when useful, differently-angled briefs). Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
